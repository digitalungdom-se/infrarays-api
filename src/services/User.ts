...
export default class User {
    ...
    public async register(userData: Omit<database.users, "id" | "recommendations" | "verified">) {
        ...
        await Promise.all([this.db.users().insert(user), sendMail(userData.email, "Verifiera din din e-postadress", body)]);
    ...
    public async uploadFile(userID: string, fileBuffer: Buffer, fileType: string, fileName: string) {
        const fileID = await this.db
            .files()
            .select("id")
            .where({ user_id: userID, type: fileType })
            .first();
        if (fileID) {
            await this.db
                .files()
                .where({ id: fileID.id })
                .update({ file: fileBuffer, created: new Date(), file_name: fileName });
    ...
    public async deleteApplication(userID: string) {
        const [user, email] = await Promise.all([
            ...
            this.db
                .emails()
                .select("content")
                .where({ type: "delete_confirmation" })
                .first(),
        ]);
    ...
    public async sendRecommendationEmail(userID: string, email: string, newEmail?: string) {
        const user = await this.db
            .users()
            .select("recommendations", "name", "id")
            .where({ id: userID })
            .first();
        ...
        const templateData = (
            await this.db
                .emails()
                .select("content")
                .where({ type: "recommendation_send" })
                .first()
        ).content;
        const template = Hogan.compile(templateData);
        const body = template.render({ token: id, name, userID: user.id });
    ...
    public async uploadRecommendationLetter(userID: string, recommendationID: string, recommendationBuffer: Buffer, fileName: string) {
        ...
        const templateDataUser = (
            await this.db
                .emails()
                .select("content")
                .where({ type: "recommendation_received" })
                .first()
        ).content;
        ...
        const templateData = (
            await this.db
                .emails()
                .select("content")
                .where({ type: "recommendation_confirmation" })
                .first()
        ).content;
        const template = Hogan.compile(templateData);
        const body = template.render({});
        ...
        await Promise.all([
            ...
            sendMail(uploaderEmail, "Rekommendationsbrev mottaget", body, [{ filename: fileName, content: recommendationBuffer }]),
        ]);
    }
    ...
    public async verify(token: string): Promise<string> {
        const verification = await this.db
            .tokens()
            .select("user_id")
            .where({ id: token, type: "verify_email" })
            .first();
    ...
    public async sendForgotPassword(email: string) {
        ...
        const [templateDataRow, tokenExists] = await Promise.all([
            ...
            this.db
                .tokens()
                .select("id")
                .where({ user_id: userID, type: "reset_password" })
                .first(),
        ]);
    ...
    public async resetPassword(token: string, password: string): Promise<string> {
        ...
        const userID = (
            await this.db
                .tokens()
                .select("user_id")
                .where({ id: tokenHash, type: "reset_password" })
                .first()
        ).user_id;
    ...
    public async getRecommendationInfo(
        userID: string,
        recommendationID: string,
    ): Promise<{ name: string; fileName?: string; received: boolean; uploaded?: Date }> {
    ...
    public async updateSurvey(userID: string, surveyData: Omit<database.surveys, "id" | "user_id">) {
        ...
        const surveyExists = await this.db
            .surveys()
            .select("id")
            .where({ user_id: userID })
            .first();
