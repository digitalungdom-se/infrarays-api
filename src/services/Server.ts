...
export default class Server {
    ...
    public async getAdminByID(id: string): Promise<database.admins> {
        return this.db
            .admins()
            .select()
            .where({ id })
    ...
    public async getAdminByEmail(email: string): Promise<database.admins> {
        return this.db
            .admins()
            .select()
            .where({ email })
    ...
    public async getFilesByUserID(
        userID: string,
        options?: { returnBuffer?: boolean; returnRecommendations?: boolean },
    ): Promise<Array<{ type: string; file: Buffer; created: Date; file_name: string }>> {
        options = options || {};
