...
export default async function (user: IUserDetails, recommendationLetters: recommendationLettersDetails, files: IFileDetails): Promise<Buffer> {
    ...
    if (user.city) {
        user.city = user.city.toLowerCase().split(' ').map((s: string) => s.charAt(0).toUpperCase() + s.substring(1)).join(' ');
    } else {
        user.city = '-';
    }

    if (user.school) {
        user.school = user.school.toLowerCase().split(' ').map((s: string) => s.charAt(0).toUpperCase() + s.substring(1)).join(' ') || '-';
    } else {
        user.school = '-';
